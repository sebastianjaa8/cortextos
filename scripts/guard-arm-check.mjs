 ...
 */
export function verdict({ stamp, bundleMtime, daemonUpSince, dirtySrcFiles, stampPresent }) {
  const lines = [];
  const findings = [];
  ...
  if (stamp.status === 'STALE') {
    ...
    // THE REASSURANCE IS GATED ON THE CAUSE, AND IT WAS NOT UNTIL 2026-08-01 (seb_boss, grading the
    // ACTIVATION 4 gate). It used to be appended to EVERY UNVERIFIABLE. build-stamp has four
    // distinct causes — no stamp, dirty src at build time, an unreachable stamp commit, and a stamp
    // written after the bundle it describes — and "expected until the first real build writes a
    // stamp" explains exactly ONE of them. Stapled to the other three it tells a reader to dismiss a
    // real, actionable provenance gap. That is the failure this file's own header warns about ("a
    // reader with no stamp reports UNVERIFIABLE forever and trains someone to ignore it") arriving
    // by the opposite road: the stamp is PRESENT and the dismissal is trained anyway.
    //
    // GATED ON A FACT, NOT ON stamp.detail. Matching the cause out of the detail STRING would be the
    // same defect one layer over — build-stamp is free to reword a detail, and this would silently
    // start explaining the wrong cause again with nothing failing. stampPresent is read from
    // readStamp() at the call site.
    const reassurance =
      stampPresent === false
        ? ` This is EXPECTED until the first real build after 2026-08-01 writes a stamp, and it will` +
          ` read as a regression. Absence of provenance is not evidence of currency.`
        : ` A stamp IS present, so this is not the pre-stamping baseline — the cause named above is` +
          ` real and actionable.`;
    findings.push(`UNVERIFIABLE-BUNDLE — ${stamp.detail}${reassurance}`);
  }
...
function selfTest() {
  ...
  const cases = [
    ...
    ['no stamp at all', { stamp: { status: 'UNVERIFIABLE', detail: 'no dist/.build-stamp.' }, bundleMtime: t('2026-07-30T02:00:00Z'), daemonUpSince: t('2026-07-30T03:00:00Z'), dirtySrcFiles: 0, stampPresent: false }, 2, 'UNVERIFIABLE-BUNDLE'],
    // A DISCRIMINATING PAIR, NOT ONE ASSERTION. Both are UNVERIFIABLE and both are exit 2, so the
    // status alone cannot tell them apart — the ONLY difference is the cause, and therefore whether
    // the "EXPECTED until the first real build" reassurance belongs. Asserting only the no-stamp
    // side would pass identically if the sentence were deleted outright; asserting only the dirty
    // side would pass if it were never shown to anyone. The pair fails in both directions.
    ['no stamp -> reassurance IS offered', { stamp: { status: 'UNVERIFIABLE', detail: 'no dist/.build-stamp.' }, bundleMtime: t('2026-07-30T02:00:00Z'), daemonUpSince: t('2026-07-30T03:00:00Z'), dirtySrcFiles: 0, stampPresent: false }, 2, 'This is EXPECTED until the first real build'],
    // The 5th element is a MUST-NOT-CONTAIN. Without it this case passes even if the reassurance is
    // appended alongside the new sentence, which is the likeliest way a future edit reintroduces the
    // bug — nobody deletes the gate, they add a branch that also emits the old text.
    ['dirty-src UNVERIFIABLE -> reassurance is NOT offered', { stamp: { status: 'UNVERIFIABLE', detail: 'bundle was built from a DIRTY src/ at 2026-08-01T18:39:17Z.' }, bundleMtime: t('2026-07-30T02:00:00Z'), daemonUpSince: t('2026-07-30T03:00:00Z'), dirtySrcFiles: 0, stampPresent: true }, 2, 'A stamp IS present', 'This is EXPECTED until the first real build'],
  ...
  let failed = 0;
  for (const [name, input, wantCode, wantText, mustNotContain] of cases) {
    const got = verdict(input);
    const text = got.lines.join('\n');
    const ok =
      got.code === wantCode &&
      text.includes(wantText) &&
      (mustNotContain === undefined || !text.includes(mustNotContain));
    if (!ok) failed++;
    console.log(`${ok ? 'ok  ' : 'FAIL'} ${name} (code ${got.code}, want ${wantCode})`);
  }
  // Both directions covered: 3 cases must come back clean, 5 must fire. A check that can only
  // ever fire is as broken as one that can never fire; it just fails loudly instead of silently.
  console.log(failed === 0 ? '\nself-test PASSED (3 clean, 5 firing)' : `\nself-test FAILED: ${failed}`);
...
try {
  ...
  // HOISTED so the raw stamp is a value this file can assert on, not just an argument it forwards.
  // verdict() needs to distinguish "UNVERIFIABLE because there is no stamp" from the three other
  // causes, and the only honest source for that is whether readStamp returned one.
  rawStamp = readStamp(REPO);
  stamp = stampVerdict({
    stamp: rawStamp,
...
const input = {
  ...
  dirtySrcFiles,
  // The FACT behind the reassurance sentence, not a re-derivation of it from stamp.detail.
  stampPresent: rawStamp !== null && rawStamp !== undefined,
};

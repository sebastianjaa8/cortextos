  ...
  async start(): Promise<void> {
    ...
    this.heartbeatTimer = setInterval(() => {
      ...
      execFile(
        ...
        { env: { ...process.env, CTX_AGENT_NAME: agentName }, windowsHide: true },
        (err) => {
...
`;
  ...
  private getCtxThresholds(): { warn: number; handoff: number } {
    ...
    const config = this.agent.getConfig();
    return {
      // ponytail: MERGE-DECISION (upstream-sync-2026-07-03) — upstream flips this to
      // default-ON (30%/60%) for every agent with no explicit config. Kept our fork's
      // existing observe-only-when-unset default instead: none of the 14 live
      // orgs/*/agents/*/config.json set ctx_handoff_threshold today, so adopting
      // upstream's default would silently turn on fleet-wide auto-restart-on-context
      // for every agent the moment this merge ships. That's a real runtime behavior
      // change, not a bug fix — flagged for Sebastian to decide explicitly rather than
      // inherit silently. To opt in per-agent, set ctx_handoff_threshold in config.json.
  ...
  private async checkContextStatus(): Promise<void> {
    ...
    // ponytail: MERGE-DECISION (upstream-sync-2026-07-03) — kept our fork's observe-only
    // default here to match getCtxThresholds above (see comment there for why). If
    // Sebastian later opts into upstream's default-ON policy, this gate should flip to
    // upstream's "opt-out via <= 0" form at the same time as the threshold defaults.

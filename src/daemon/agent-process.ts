...
export class AgentProcess {
  ...
  injectMessageDetailed(
    ...
    // ponytail: MERGE-DECISION (upstream-sync-2026-07-03) — upstream dispatches through
    // `this.pty.injectMessage()` when the PTY class defines one (needed for OpencodePTY's
    // TUI-specific key handling), else falls through to the bare inject.ts call. That bare
    // duck-typed dispatch would also catch AgentPTY's own new `injectMessage()` method
    // (added by upstream, see src/pty/agent-pty.ts) and silently skip our verified-submit
    // Enter-swallow retry (getOutputBytes/onFailed/dedup-un-poisoning, fixed 2026-07-01/02)
